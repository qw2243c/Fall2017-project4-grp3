...
```{r}
#Training function
cluster.model.train <- function(data.set, C = 3, replace.na = T, tau = 0.1, max.iter = 15){
...
cluster.model.test <- function(data.set.test, lambda, mu, pi, websites, replace.na = T, alpha = 5){
  ...
  for(i in 1:nrow(predictions)){
    ...
    jay2 <- 1:sum(data.set.test[i,])
    ra[i,2] <- sum(rep(1,sum(data.set.test[i,]))/(2^((jay2-1)/(alpha-1))))
  }
