...
```{r}
#Training function
cluster.model.train <- function(data.set, C = 3, replace.na = T, tau = 0.1, max.iter = 20){
...
#Testing function
cluster.model.test <- function(data.set.test, lambda, mu, pi, websites, replace.na = T){
  ...
  for(i in 1:N){
    for(j in 1:ncol(predictions)){
      ...
      predictions[i,j] <- which.max(lambda[, index, cluster]) - 1    
      
      error <- error + ifelse(predictions[i,j] == data.set.test[i,j], 0, 1)
    }
  }
  
  error.rate <- error/(N * ncol(data.set.test))
  #------------------------------------------------------
  
  return(list("predictions" = predictions, "error" = error, "error.rate" = error.rate))
}
```
...
if(model.clustering){
  ...
  trained.cluster.model <- cluster.model.train(ms.data.train, C = 3)
  ...
  testing <- cluster.model.test(ms.data.test, lambda = l, mu = m, pi = p, websites = w)
  
  paste("Error rate:",round(testing$error.rate * 100,2), "%")
