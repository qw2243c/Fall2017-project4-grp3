...
```{r}
  sim.rank.model <- function(movie.data.train) {
  ...
  for (run in 1:times){
    ...
    write.csv(user_sim, file='../output/usersim.csv')
    return(user_sim)
...
sample.data.set <- movie.data.train[1:100, 1:100]

user.sim <- sim.rank.model(sample.data.set)
}
```
